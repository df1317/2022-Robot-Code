...
- ADD CODE TO CLEAR STICTY FAULTS see documentation
 */
...
public class Robot extends TimedRobot {
  ...
  double rightValue;
  double extraValue;
  double manualShooterSpeed; // Controlled by flap on operator joystick, hopefully rely on limelight 98% of
                             // time
  ...
  public void autonomousPeriodic() {
    ...
    /*
     * Simple move action, any position. We will have to face the robot in the
     * desired direction at competition, time and speed values
    ...
    if (selectedAction.equals("3")) {
      if (selectedPosition.equals("1")) {
        if (currentAutoTime > 10) {
          ...
          leftValue = 0.1;
          /*
           * At this point the limelight will override robot turning when it sees the
  ...
  public void teleopPeriodic() {
    ...
    boolean joyE11Button = joyE.getRawButton(11);
    boolean joyE10Button = joyE.getRawButton(10);
    double speedMultiplier = 0.8;

    //TESTING ONLY
    if (joyE11Button == true) {
      ACTMotor.set(0.5);
    }
    if (joyE10Button == true) {
      ACTMotor.set(-0.5);
    }
    //TESTING ONLY
